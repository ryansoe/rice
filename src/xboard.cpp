...
int checkResult(S_BOARD *pos) {
    if (pos->fiftyMove > 100) {
        printf("1/2-1/2 {fifty move rule (claimed by Rice)}\n"); return TRUE;
    }

    if (threeFoldRep(pos) >= 2) {
        printf("1/2-1/2 {3-fold repetition (claimed by Rice)}\n"); return TRUE;
    }

    if (drawMaterial(pos) == TRUE) {
        printf("1/2-1/2 {insufficient material (claimed by Rice)}\n"); return TRUE;
    }
    ...
    if(inCheck == TRUE) {
        if(pos->side == WHITE) {
            printf("0-1 (black mates {claimed by Rice})\n"); return TRUE;
        } else {
            printf("0-1 (white mates {claimed by Rice})\n"); return TRUE;
        }
    } else {
        printf("1/2-1/2 (stalemate {claimed by Rice})\n"); return TRUE;
    }
    return FALSE;
}
...
void console_loop(S_BOARD *pos, S_SEARCHINFO *info) {
	...
	printf("Welcome to Rice In Console Mode!\n");
